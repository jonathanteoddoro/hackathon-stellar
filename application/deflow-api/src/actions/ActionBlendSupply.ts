...
export class ActionBlendSupply extends ActionNode {
    name = 'ActionBlendSupply';
    description = 'Executes supply operation on Blend Protocol on Stellar Testnet';
    ...
    constructor(params: BlendActionParams) {
        ...
        if (!params || !params.userAddress) {
            throw new Error('ActionBlendSupply requires a valid userAddress in constructor');
        }
        this.params = params;
    ...
    async execute(message: NodeMessage): Promise<NodeMessage> {
        ...
        try {
            ...
            const result = await this.supplyToBlend(amount, asset);
    ...
    private async supplyToBlend(amount: number, asset: string): Promise<any> {
        ...
        const command = `stellar contract invoke --id ${this.contractId} --source-account default --network testnet --send=yes -- supply --supplier ${this.params.userAddress} --amount ${amount} --asset "${asset}"`;
