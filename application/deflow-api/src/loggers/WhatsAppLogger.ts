...
export class WhatsAppLogger extends LoggerNode {
    ...
    constructor(params: Record<string, string>) {
        ...
        if (!this.fromNumber) {
            throw new Error('WhatsAppLogger requires a from number (params.from or TWILIO_WHATSAPP_FROM)');
        }
        if (!this.toNumber) {
            throw new Error('WhatsAppLogger requires a to number (params.toNumber or TWILIO_WHATSAPP_TO)');
        }
    }
